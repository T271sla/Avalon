...
public abstract class Tile : MonoBehaviour
{
    ...
    private void OnMouseEnter()
    {
        ...
        if(!MenuManager.Instance.battleUIActive && GameManager.Instance.GameState == GameState.HeroesTurn) 
        {
    ...
    {
        if (GameManager.Instance.GameState != GameState.HeroesTurn || MenuManager.Instance.battleUIActive) return;
        ...
        if(OccupiedUnit != null)
        {
            if (OccupiedUnit.Faction == Faction.Hero && UnitManager.Instance.selectedHero != OccupiedUnit && OccupiedUnit.active)
            {
                ...
                UnitManager.Instance.selectedHero.validMovementTiles = UnitManager.Instance.selectedHero.findValidMovement(this, 0);
            }
            else
            {
                if (UnitManager.Instance.selectedHero != null && OccupiedUnit.Faction != UnitManager.Instance.selectedHero.Faction && MenuManager.Instance.attackPressed)
                {
                    ...
                    if(distanceAttackX + distanceAttackY <= UnitManager.Instance.selectedHero.unitRange)
                    {
                        ...
                        MenuManager.Instance.subMenuActivity(false);
